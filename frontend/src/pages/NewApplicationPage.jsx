      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full py-3 pr-4 bg-surface border border-outline-variant rounded-lg
            text-[14px] font-medium text-on-surface focus:ring-2 focus:ring-primary
            focus:border-primary outline-none transition-all placeholder:text-outline
  ...
  return (
    <div
      className={`rounded-xl px-6 py-5 flex items-center justify-between gap-4 flex-wrap
        ${approved ? "bg-emerald-50 border border-emerald-200" : "bg-red-50 border border-red-200"}`}
    ...
    >
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          <p
            className={`text-[14px] mt-1 ${approved ? "text-emerald-700" : "text-red-700"}`}
          >
            {approved
...
/* ─── NEW: ResultMetricsRow — Risk Tier + Recommended Amount ─── */
function ResultMetricsRow({ result }) {
    ...
    >
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <span
            className={`px-3 py-1.5 rounded-full text-[13px] font-bold inline-flex items-center gap-1.5
            ${approved ? "bg-emerald-100 text-emerald-800" : "bg-error-container text-on-error-container"}`}
          >
            <span
              className={`w-2 h-2 rounded-full ${approved ? "bg-emerald-500" : "bg-error"}`}
            />
            {result.status}
          </span>
        </div>
        <p className="text-[11px] text-on-surface-variant mt-3 leading-relaxed">
          {approved
            ? "Cleared for underwriting review."
            : "Application did not meet thresholds."}
        </p>
      </div>
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 shadow-sm">
        ...
        <p className="text-[20px] font-bold text-primary leading-tight tracking-tight">
          {result.risk_tier || "Tier Unassigned"}
        </p>
        <p className="text-[11px] text-on-surface-variant mt-2 leading-relaxed">
          AI-assigned risk category based on applicant profile and model
          weights.
        </p>
      </div>

      {/* Recommended Amount — NEW */}
      <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 shadow-sm">
        <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-3 flex items-center gap-2">
        ...
        <p
          className={`text-[24px] font-bold leading-none tracking-tight font-mono
          ${
            result.recommended_amount !== undefined &&
        ...
        >
          {result.recommended_amount !== undefined
            ? fmt$(result.recommended_amount)
            : "N/A"}
        </p>
        <p className="text-[11px] text-on-surface-variant mt-2 leading-relaxed">
          Maximum loan amount the model recommends approving.
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex items-center gap-2 z-10 mt-auto">
        <div className="flex items-end gap-0.5 h-5">
          {[3, 5, 4, 2, 4, 3, 5].map((h, i) => (
            <div
              key={i}
              className={`w-1 rounded-full ${approved ? "bg-emerald-400" : "bg-red-400"}`}
              style={{
                height: `${h * 3}px`,
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-1.5">
        ...
        <span
          className={`text-[13px] font-bold font-mono ${isPos ? "text-emerald-600" : "text-error"}`}
        >
          {isPos ? "+" : ""}
          {value.toFixed(3)}
        </span>
    ...
    <div className="space-y-5" style={{ animation: "fadeUp 0.35s ease both" }}>
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 border border-outline-variant rounded-lg
            text-[12px] font-bold uppercase tracking-widest text-on-surface-variant
            hover:bg-surface-variant transition-all flex-shrink-0"
        >
      ...
      >
        ...
        />
        <div
          className={`w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center ${file ? "bg-emerald-100" : "bg-surface-container-high"}`}
        >
          <span
            className={`material-symbols-outlined text-[28px] ${file ? "text-emerald-600" : isDragOver ? "text-primary" : "text-on-surface-variant"}`}
          >
      ...
      {status === "uploading" && (
        <div
          className="flex items-center gap-4 bg-surface-container-low border border-outline-variant rounded-xl px-5 py-4"
      ...
      {status === "success" && response && (
        ...
        >
          <div className="flex items-start gap-4 bg-emerald-50 border border-emerald-200 rounded-xl px-5 py-4">
            ...
            <button
              onClick={handleReset}
              className="flex items-center gap-1.5 px-3 py-1.5 border border-emerald-300 rounded-lg
                text-[11px] font-bold uppercase tracking-wider text-emerald-700 hover:bg-emerald-100 transition-colors flex-shrink-0"
            >
          ...
          {(response.total != null ||
            ...
            <div className="grid grid-cols-3 divide-x divide-outline-variant border border-outline-variant rounded-xl overflow-hidden">
              {[
                {
                  label: "Total",
                ...
                <div
                  key={label}
                  className="px-5 py-4 text-center bg-surface-container-lowest"
                >
          ...
          <pre className="text-[12px] font-mono text-on-surface-variant bg-surface-container rounded-xl p-4 overflow-x-auto leading-relaxed">
      ...
      {status === "error" && (
        <div
          className="flex items-start gap-3 bg-error-container text-on-error-container rounded-xl px-5 py-4 border border-error"
          style={{ animation: "fadeUp 0.3s ease both" }}
        >
          ...
          <button
            onClick={handleReset}
            className="flex items-center gap-1 px-3 py-1.5 bg-error text-on-error rounded-lg text-[11px] font-bold uppercase tracking-wider flex-shrink-0"
          >
            <span className="material-symbols-outlined text-[15px]">
              refresh
            </span>
      ...
      {file && status !== "uploading" && status !== "success" && (
        ...
        >
          <button
            ...
            className="flex items-center gap-2 bg-primary text-on-primary px-7 py-3 rounded-lg
              text-[13px] font-bold uppercase tracking-widest hover:opacity-90 active:scale-[0.98] transition-all shadow-md"
          >
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {/* Tabs */}
        <div className="flex items-center gap-1 bg-surface-container-low border border-outline-variant rounded-xl p-1 w-fit">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg text-[13px] font-bold uppercase tracking-wider transition-all
                ${activeTab === tab.id ? "bg-primary text-on-primary shadow-sm" : "text-on-surface-variant hover:text-on-surface hover:bg-surface-variant"}`}
            >
              <span className="material-symbols-outlined text-[18px]">
                {tab.icon}
              </span>
          ...
          >
            {result && (
              <div className="flex justify-end">
                <button
                  onClick={handleReset}
                  className="flex items-center gap-2 px-5 py-2.5 border border-outline text-on-surface-variant
                    rounded-lg hover:bg-surface-variant transition-all text-[12px] font-bold uppercase tracking-widest"
                >
                  <span className="material-symbols-outlined text-[18px]">
                    restart_alt
                  </span>
            ...
            {/* Form */}
            <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-[0_4px_20px_-2px_rgba(0,0,0,0.06)] p-6">
              {result && (
                <p className="text-[11px] font-bold uppercase tracking-widest text-on-surface-variant mb-4 flex items-center gap-2">
                  <span className="material-symbols-outlined text-[16px]">
                    edit
                  </span>
              ...
              {error && (
                <div className="mt-5 flex items-center gap-2 bg-error-container text-on-error-container rounded-lg px-4 py-3 text-[13px]">
                  <span className="material-symbols-outlined text-[18px]">
                    error
                  </span>
                  {error}
              ...
              <div className="mt-6 flex justify-end">
                <button
                  ...
                  disabled={loading}
                  className="bg-primary text-on-primary px-7 py-3 rounded-lg text-[13px] font-bold uppercase tracking-widest
                    hover:opacity-90 active:scale-[0.98] transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            {result && (
              ...
              >
                ...
                {/* 2 — NEW: System Decision + Risk Tier + Recommended Amount row */}
                <ResultMetricsRow result={result} />
                ...
                {/* 4 — Primary deciding factor */}
                <div
                  className="flex items-center gap-3 bg-surface-container-lowest border border-outline-variant
                  rounded-xl px-5 py-4 shadow-[0_4px_20px_-2px_rgba(0,0,0,0.04)]"
                  style={{
                    animation: "fadeUp 0.4s ease both",
